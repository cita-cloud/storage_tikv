...
const GIT_HOMEPAGE: &str = "https://github.com/cita-cloud/storage_tikv";
...
struct RunOpts {
    ...
    grpc_port: String,
    /// Sets db path.
    #[clap(short = 'e', long = "pd_endpoints", default_value = "127.0.0.1:2379")]
    pd_endpoints: String,
}
...
fn main() {
    ...
    match opts.subcmd {
        ...
        SubCommand::Run(opts) => {
            ...
            info!("pd_endpoints: {}", opts.pd_endpoints);
            let _ = run(opts);
        }
    }
}
...
impl StorageService for StorageServer {
    async fn store(&self, request: Request<Content>) -> Result<Response<SimpleResponse>, Status> {
        ...
        let ret = self.db.store(region, key, value).await;
        if ret.is_err() {
    ...
    async fn delete(&self, request: Request<ExtKey>) -> Result<Response<SimpleResponse>, Status> {
        ...
        let ret = self.db.delete(region, key).await;
        if ret.is_err() {
...
async fn run(opts: RunOpts) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // init db
    let db = DB::new(&opts.pd_endpoints).await;
